...
Before you can give yourself Anypoint MQ access permissions, create an environment with which to associate permissions.

. From Anypoint Platform, select *Access Management*
. Click *Environments* in the left menu, and click the *Add environment* button.
. In the *Add environment* popup, provide a name for your environment, and click either *Production*, *Sandbox*, or *Design*. You might want to create separate environments for each. In this case, choose *Production*.
...
Creating a role lets you assign access rights to users in your organization, such as for administrators, developers, or users who only view information but don't change it.
...
. From the *Environment* drop-down, select *Production*.
. From the *Permissions* drop-down, select *Select all*.
