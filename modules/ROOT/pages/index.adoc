...
Anypoint MQ supports hybrid use cases and IoT (Internet of Things), in which you collect data from different devices.
Anypoint MQ also provides a REST API for device applications.
You can also use Anypoint MQ with other app frameworks such as node.js to communicate with queues.

This spotlight video gives an overview of Anypoint MQ:
...
. Make sure you have a paid Anypoint Platform package or subscription with the Anypoint MQ integration add-on.
+
Anypoint MQ isn't available in the trial edition of Anypoint Platform.
+
To add Anypoint MQ to your package or subscription, contact your account team.
After purchasing Anypoint MQ, wait one hour for Anypoint MQ to become available in Anypoint Platform.
+
For information about Anypoint Integration packages, see 
https://www.mulesoft.com/anypoint-pricing[MuleSoft Anypoint Platform Pricing].
...
You can't use connected apps across multiple environments or business groups.
+
...
image::mq-pub-q-sub.png["A publisher sends a message to a queue, and a subscriber reads messages from a queue"]

To distribute messages to multiple queues and consumers, bind multiple queues to a _message exchange_.
By default, messages sent to an exchange appear in all of its bound queues simultaneously.
When you configure message-routing rules, a message appears only in queues whose binding
has no rule or has a rule that the message matches.
For details, including what happens when no rule matches, see xref:mq-routing-rules.adoc[].

image::mq-pub-ex-sub.png["A publisher sends messages to an Exchange, which then directs messages to queues, from which consumers retrieve them"]
...
image::mq-crm-to-dbs.png["A CRM system communicates customer updates through a message exchange, which then relays the information to separate queues for a database and SAP"]
...
*** If a consumer can't process a message, the message proceeds with a negative acknowledgment and becomes visible in the queue again.
--
+
...
[[mq-features-control-plane]]
== Anypoint MQ Features by Control Plane
...
* xref:mq-queues.adoc[Encrypted Queues]
* xref:mq-exchanges.adoc[Message Exchanges]
* xref:mq-routing-rules.adoc[Message Routing]
* xref:mq-connected-apps.adoc[Connected Apps]
* xref:mq-connectors.adoc[Anypoint MQ Connector versions 4.x and later]
* xref:mq-apis.adoc#mq-admin-api[Anypoint MQ Admin API]
* xref:mq-apis.adoc#mq-broker-api[Anypoint MQ Broker API]
* xref:mq-apis.adoc#mq-stats-api[Anypoint MQ Stats API] (Anypoint MQ Stats only)
...
| *xref:cloudhub::deploying-to-cloudhub.adoc[CloudHub 1.0 Deployments]*
| Supported
| Not supported. Deploy apps to CloudHub 2.0 shared spaces or Anypoint Runtime Fabric.

| *xref:mq-client-apps.adoc[Client Apps]*
| Not supported. Use xref:mq-connected-apps.adoc[connected apps] instead.
| Not supported. Use xref:mq-connected-apps.adoc[connected apps] instead.

| *xref:mq-connectors.adoc[Anypoint MQ Connector earlier than 4.x]*
| Not supported. Requires xref:anypoint-mq-connector::index.adoc[Anypoint MQ Connector 4.x for Mule 4].
| Not supported. Requires xref:anypoint-mq-connector::index.adoc[Anypoint MQ Connector 4.x for Mule 4].
...
| *xref:mq-usage.adoc#access-mq-usage-charts[Usage Charts] (Access Management)*
| Supported
| Not available. Use xref:mq-usage.adoc#access-mq-usage-reports[Usage Reports] instead.

| *xref:mq-apis.adoc#mq-stats-api[Anypoint MQ Stats API] (Usage Metrics)*
| Not supported. Use the Stats API only for xref:mq-apis.adoc#historical-stats-queue[queue] and xref:mq-apis.adoc#historical-stats-exchange[exchange] metrics.
| Not supported. Use the Stats API only for xref:mq-apis.adoc#historical-stats-queue[queue] and xref:mq-apis.adoc#historical-stats-exchange[exchange] metrics.

|===

For information about the regions where Anypoint MQ is available, see xref:mq-faq.adoc#mq-regions[Anypoint MQ Regions].

For information about MuleSoft Government Cloud, see xref:gov-cloud::index.adoc[].
