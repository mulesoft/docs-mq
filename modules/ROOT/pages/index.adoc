...
Anypoint MQ supports hybrid use cases, IoT (Internet of Things) in which you collect data from different devices, and a REST API you can use with device applications.
...
* Supports using xref:mq-connected-apps.adoc[connected apps] to authenticate with Anypoint Platform using APIs.
...
Usage Information::
Organization administrators or owners can view the current and past months' usage, including the total number of messages and API requests. You access this information from the *Access Management* page. For more information, see xref:mq-usage.adoc[].
REST API::
...
Message _queues_ provide temporary storage that enables applications to communicate in a scalable and reliable way. Publishers can publish messages into queues, and subscribers can receive messages from queues. 

image::mq-pub-q-sub.png[Anypoint MQ Publisher-Queue-Subscriber]

You can create a dead-letter queue to store undeliverable messages.

Use a _message exchange_ to bind one or more queues so that a message sent to a message exchange appears in all its bound queues simultaneously.
