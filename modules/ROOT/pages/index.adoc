...
Send messages to queues, pull messages from queues.
Use a message exchange to perform pub/sub scenarios to send a message to multiple queues.
+
For information, see xref:mq-queues.adoc[] and xref:mq-exchanges.adoc[].
Management Console::
...
Anypoint MQ is fully integrated with Anypoint Platform access management, allowing you to specify different environments for your queues, who can access each environment, and what they can do within each environment.
+
For information, see xref:mq-access-management.adoc[].
Client Management::
