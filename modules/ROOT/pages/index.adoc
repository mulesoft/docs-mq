...
+
In addition, you can enable failover on standard queues, which creates a fallback queue in a different region.
With failover enabled, if an app can't contact the Anypoint MQ server in a region,
it switches to the fallback queue to continue publishing and consuming messages.
