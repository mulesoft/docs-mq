...
* The access token for any MQ API has a default time to live (TTL) of 180 minutes. This is the same TTL that users get when they log into the Anypoint Platform. The TTL is set in the Access Management screen for an organization in the Default Session Timeout value.
...
=== Admin API and Stats API Access Token

For the Admin API and Stats API, get the access token from the Anypoint
Platform user with this curl command:
