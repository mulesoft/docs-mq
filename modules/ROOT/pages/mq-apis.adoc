...
:page-notice-banner-message: The limits described under <a href="https://docs.mulesoft.com/mq/mq-apis#usage-limits" target="_blank" rel="noopener">Usage Metrics Limits</a> aren't enforced yet. Take appropriate action to adapt. See <a href="https://help.salesforce.com/s/articleView?id=004754267&type=1" target="_blank" rel="noopener">Changes to MQ and Object Store v2 Statistics API</a>. For information about Anypoint MQ Stats API rate limits, see <a href="https://docs.mulesoft.com/mq/mq-apis#mq-stats-api" target="_blank" rel="noopener">Anypoint MQ Stats API</a>.
...
Anypoint MQ APIs are available in the https://dev-portal.mulesoft.com/index.html?filter=api[MuleSoft developer portal].
...
To access the Anypoint MQ APIs, you must first get the bearer token from the Anypoint Platform user using the https://dev-portal.mulesoft.com/apis/access-management.html[Access Management API].
...
. Run this command, replacing AUTH_SERVER_HOST with the authorization server, for example `\https://anypoint.mulesoft.com/`
+
[source,bash,linenums]
----
curl -H \
  ...
  "https://<AUTH_SERVER_HOST>/accounts/login"
----
+
...
IMPORTANT: You can use the broker bearer token only with the Broker API.
Don't use this token with the Stats and Admin APIs.
Instead, follow the steps in <<get-bearer-token>> to get the bearer token.
...
You can use the
https://dev-portal.mulesoft.com/apis/anypoint-mq-admin.html[Anypoint MQ Admin API] to create and manage queues, message exchanges, and client apps.
...
[NOTE]
FIFO exchanges don't support routing rules. To use routing rules, configure a standard exchange.
...
[IMPORTANT]
include::partial$mq-msg-exchange.adoc[tag=msgPublishRoutingOutcome]
...
see this endpoint in the https://dev-portal.mulesoft.com/apis/anypoint-mq-admin.html[Anypoint MQ Admin API].
...
The
https://dev-portal.mulesoft.com/apis/anypoint-mq-broker.html[Anypoint MQ Broker API^] enables clients to publish, consume, route, and acknowledge messages from queues and message exchanges.
...
For the regions where Anypoint MQ and FIFO are available, see xref:mq-faq.adoc#mq-regions[Anypoint MQ Regions].
...
This `curl` command publishes a message with the value of the `itemCategory`
property as `premium`.
...
You can use the https://dev-portal.mulesoft.com/apis/anypoint-mq-stats.html[Anypoint MQ Stats API] to perform statistical analysis of queue performance.

Use the Stats API to retrieve:

* Anypoint MQ statistics:
** <<get-queue-stats,Near real-time statistics for queues>>
** Number of messages currently in a queue
** <<historical-stats-queue,Historical statistics for queues>>
** <<historical-stats-exchange,Historical statistics for message exchanges>>
* Usage metrics
+
View your entire organization's usage of Anypoint MQ for billing purposes (US and EU Cloud only):
+
--
** <<usage-by-env,Usage per environment>>
** <<usage-by-org,Usage per organization>>
--
...
For more information, see the https://dev-portal.mulesoft.com/apis/anypoint-mq-stats.html[Anypoint MQ Stats API] in the MuleSoft developer portal.
...
[NOTE]
// Not on HF
include::partial$mq-on-hyperforce.adoc[tag=mqStatsAPInotHF]
...
NOTE: To access the Anypoint MQ Stats API, you must first authenticate using the https://dev-portal.mulesoft.com/apis/access-management.html[Access Management API].
...
This example consists of the `cloudhub.properties` file in which
...
This example properties file defines the access credentials,
...
For the regions where Anypoint MQ and FIFO queues are available,
see xref:mq-faq.adoc#mq-regions[Anypoint MQ Regions].
...
The https://www.postman.com/[Postman^] app provides a platform for accessing the Anypoint MQ APIs.
...
To download the Anypoint MQ APIs from Exchange, click *Download* and select the format to download: RAML, OAS, or as a connector file for Mule 4.

image::mq-exchange-download.png["Download option for Anypoint MQ Admin API in Exchange"]

== See Also

* xref:mq-faq.adoc[Anypoint MQ FAQ]
* https://dev-portal.mulesoft.com/apis/anypoint-mq-admin.html[Anypoint MQ Admin API]
* https://dev-portal.mulesoft.com/apis/anypoint-mq-broker.html[Anypoint MQ Broker API]
* https://dev-portal.mulesoft.com/apis/anypoint-mq-stats.html[Anypoint MQ Stats API]
