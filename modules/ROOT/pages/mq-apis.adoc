...
These examples use the Anypoint MQ Broker API to publish and consume messages and test Anypoint MQ from the command line using curl.
...
If the value of `payloadVisibility` is `none`, the response includes `payloadHidden=true` in the header.

The following curl command gets a message and only displays the payload if the payload size is smaller than 1 MB:
