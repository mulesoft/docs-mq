...
* Anypoint MQ metrics:
** Almost real-time statistics for queues
...
Use the Stats API only for statistics.
The response from the Stats API  includes 60 seconds of inconsistency due
to internal post-processing time when the status of a message changes (to in-flight, from in-flight, and to deleted).
For this reason, don't use the Stats API for syncing processing actions,
such as checking if the queue is empty, because it can report 60-second inconsistencies after any message status change.

You can set a 60-second delay to account for the post-processing time.
