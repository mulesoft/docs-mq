...
==== Retrieve Failover Configuration
...
<.> The fallback queue for this queue is `00-test_fb`.
...
<.> Failover is not enabled for this queue.
...
+
Response includes `"isFallback" : false` but not `fallbackConfig`.
+
====
[source,json]
----
...
FIFO Queue::
+
Response doesn't include `fallbackConfig` because FIFO queues don't support failover.
...
Exchange::
+
Response doesn't include `fallbackConfig` because exchanges don't support failover.
+
====
[source,json]
----
...
----
<.> This queue is a message exchange. 
====
