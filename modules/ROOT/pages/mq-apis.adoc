...
The following example with the Anypoint MQ Broker API illustrates the workflow for accessing an API:
...
** <<mq-broker-api>>:
+
Get a token from Anypoint MQ by supplying the client ID and client secret for an Anypoint MQ app or connected app.
** <<mq-stats-api>>:
+
Get a bearer token for an Anypoint Platform user using the access management API.
* Get the organization ID (ORG_ID) and environment ID (ENV_ID).
+
...
+
See <<get-region-id>>.
* Get the queue ID (QUEUE_ID).
+
You need the queue ID for operations with the MQ Admin API.
+
See <<get-queue-id>>.
...
[[get-bearer-token]]
=== Get a Bearer Token to Authenticate with the Anypoint MQ Admin API
...
To use the Admin API, you can get the bearer token from the Anypoint Platform user.
The default time to live (TTL) for the bearer token is 60 minutes.

You can get the organization ID and environment ID in the `payload.txt` file that Anypoint Platform creates for any auditable action in Anypoint MQ.
See xref:access-management::audit-logging.adoc#to-query-audit-logging-for-anypoint-mq[MQ Audit Logging].


[[get-broker-token]]
=== Get the Broker Access Token

The broker access token in this section can only be used with the Broker API. Do not use this access token 
...
. Click *MQ* > *Client Apps*, and then click the *Add* icon (image:icon-blue-plus.png["",3%,3%]) to create an app.
...
----
{"access_token":"<token>","simple_client":{"envId":"ENV_ID","orgId":"ORG_ID"},"token_type":"bearer"}
----
...
. xref:mq-connected-apps.adoc[Configure a connected app].
+
...
=== Get the Bearer Token to Authenticate with the Anypoint MQ Stats API

To access the Anypoint MQ Stats API, you must first get the bearer token from the Anypoint Platform user using the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/access-management-api/[Access Management API].
...
+
Use this client ID (CLIENT_ID) and client secret (CLIENT_SECRET) to get the bearer token required to authenticate with the Anypoint MQ Stats API.
...
You can get the organization ID (ORG_ID) and environment ID (ENV_ID) in the `payload.txt` file that Anypoint Platform creates for any auditable action in Anypoint MQ.
See xref:access-management::audit-logging.adoc#to-query-audit-logging-for-anypoint-mq[Anypoint MQ Audit Logging]. 
...
You must substitute the placeholder string, `ORG_ID` with the proper values for authorization and token strings. These examples use Postman to access the queue.
