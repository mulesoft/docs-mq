...
Each time an app accesses the token before its expiry extends the token validity by 60 minutes.
If multiple apps use the same `CLIENT_ID`, each access extends the token validity.
