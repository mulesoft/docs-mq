...
https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/anypoint-mq-stats/[Statistics API] - Perform statistical analysis of queue performance.

In the Statistics API portal, click *Download* to download the API as a RAML, Mule 3 zipped connector file, or as a Mule 4 connector JAR file.
...
Anypoint MQ uses the Mule Analytics service to store usage metric data. The Statistics API can be used to sum up the usage for Anypoint MQ across all environments for a particular organization. This is not real time and has a latency.
...
Anypoint MQ uses the Mule Analytics service to store usage metric data for an organization. The Statistics API can be used to sum up the usage for Anypoint MQ across the master business organization:
...
[[exstatapi]]
=== Example: Anypoint MQ Statistics API
