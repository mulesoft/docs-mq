...
. Run the following command, replacing AUTH_SERVER_HOST with the authorization server, for example `\https://anypoint.mulesoft.com/`
+
[source,bash,linenums]
----
curl -H \
...
To retrieve these usage metrics on Canada Cloud and Japan Cloud, see 
xref:mq-usage-api.adoc#retrieve-metrics-metering-api[Retrieve Usage Metrics with Anypoint Metering Query API].
...
[NOTE]
include::partial$mq-api-shared.adoc[tag=mqStatsAPIlimits]
To retrieve these usage metrics on Hyperforce, see 
xref:mq-usage.adoc#retrieve-metrics-metering-api[Retrieve Usage Metrics with Anypoint Metering Query API].
