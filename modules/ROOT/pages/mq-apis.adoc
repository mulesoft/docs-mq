...
You can control whether to show the message payload in the response using the `showPayload` query parameter.
This parameter is useful when retrieving large payloads.

The `showPayload` parameter takes the following options: 

* `true`: Shows the message payload in the response+
This option is the default.
* `false`: Hides the message payload in the response
* `partial`: Hides the message payload if the payload size is larger than 1 MB. 
