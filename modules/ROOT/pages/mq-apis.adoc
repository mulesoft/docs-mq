...
[[getoeb]]
== Getting an Organization ID, Environment ID, and Access Token
...
== Creating a FIFO Queue in the Admin API Portal

You can create a FIFO queue from the Admin API portal by including the new `"fifo": true` field.

*Note:* The organization from which you use the Administration portal must have an Anypoint MQ FIFO entitlement. See xref:index.adoc[Anypoint MQ] for the regions MQ is available in.

If needed, see <<getoeb,Getting an Organization ID, Environment ID, and Bearer Token>>.
