...
The following `curl` command creates a routing rule:
...
If the value of the `itemCategory` property on the message matches `premium`,
the rule on the `purchases` exchange binding routes the message to the `premiumPurchases` queue.
...
property as `premium`.
The rule on the `purchases` exchange binding routes the message to the `premiumPurchases` queue set up in
<<routing-rules-api>>.
