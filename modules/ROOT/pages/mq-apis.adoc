...
* <<mqadminapi,Admin API>>
* <<mqbrokerapi,Broker API>>
* <<mqstatapi,Stats API>>
...
Get the access token from the Anypoint Platform user. The access token is also known as the Bearer.

The access token has a default time to live (TTL) of 60 minutes. The TTL duration is set in the Access Management 
screen for an organization in the Default Session Timeout value and can range from 15 minutes to 180 minutes.

You can get the organization ID and environment ID in the `payload.txt` file that Anypoint Platform creates for any auditable action in MQ. See xref:access-management::audit-logging.adoc#to-configure-anypoint-mq-audit-logging[MQ Audit Logging]. 
...
=== Create a FIFO Queue in the Admin API
...
*Note:*  The access token in this section can only be used with the Broker API. Do not use this access token 
with the Stats and Admin APIs. See <<Admin API and Stats API Access Token>> to get that access token.
...
In the Broker API, click Download to download the RAML or the API as a Mule Plugin.
...
In the Stats API, click Download to download the RAML or the API as a Mule Plugin.
...
----
{
  ...
  "messages": [
    {
      "date": "2018-10-26T00:00:00.000+0000",
      "value": 2126
    },
    {
      "date": "2018-10-27T00:00:00.000+0000",
      "value": 2126
    },
    {
      "date": "2018-10-28T00:00:00.000+0000",
      "value": 587
    }
  ],
  "inflightMessages": [
...
----
{
    ...
    "headers": {
      "messageId": "514",
      "lockId": "<lockIDvalue>",
      "created": "Fri, 23 Oct 2018 21:17:57 GMT",
      "deliveryCount": "2"
    },
    ...
----
