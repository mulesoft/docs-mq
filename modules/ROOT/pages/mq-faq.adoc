...
.7+| US .3+| North America | Northern Virginia (`us-east-1`)
                  | Oregon (`us-west-2`)
                  | Central Canada (`ca-central-1`)
    .2+| Europe     | Ireland (`eu-west-1`)
                    | London (`eu-west-2`)
   .2+| APAC        | Singapore (`ap-southeast-1`)
                    | Sydney (`ap-southeast-2`)
.2+| EU .2+| Europe | Frankfurt (`eu-central-1`)
                    | Ireland (`eu-west-1`)
|===
...
Anypoint MQ messages are always transported using in-flight HTTPS encryption with the latest Transport Layer Security (TLS) protocols.
//Shared Encryption
...
See xref:mq-apis.adoc#example-retrieve-a-message[Example: Retrieve a Message].
...
There is no maximum number of transactions per second (TPS) for standard queues or exchanges.

FIFO queues have a limit of 300 TPS. If you batch 10 messages per read and write operation (maximum) using the API, FIFO queues can support up to 3,000 TPS.
...
Using the Anypoint MQ Broker API, you can batch-upload messages to a standard or FIFO queue.

Use the `batchSize` query parameter to specify the number of messages to retrieve.
You can retrieve a maximum of 10 messages in a single call.

You can configure a lower value using the `Prefetch Config` parameter in Anypoint MQ Connector.
The number of messages retrieved by the connector is sometimes fewer than the number configured. 
A number higher than 10 is overridden.
...
and xref:connectors::anypoint-mq/3.x/anypoint-mq-listener.adoc[Anypoint MQ Subscriber Source - Mule 4].
