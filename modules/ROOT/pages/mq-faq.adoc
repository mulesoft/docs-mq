...
.8+| US .3+| North America | Northern Virginia (`us-east-1`)
    ...
    .2+| Europe     | Ireland (`eu-west-1`)
                    | London (`eu-west-2`)
   .3+| APAC        | Singapore (`ap-southeast-1`)
...
== How do I pass a message identifier between different systems?

Because the correlation ID associated with a message differs between types of flows, you must create a custom property that assigns consistent correlation IDs using message information (such as headers or payloads) that is the same between systems. When you create a single request ID for identifying messages, write your flows to ensure the request ID is readable to other components. For more information, see https://blogs.mulesoft.com/dev-guides/how-to-tutorials/total-traceability/[Total Tracability with Correlation IDs].
