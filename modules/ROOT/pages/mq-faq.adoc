...
== How can I send a hashmap as a message?

Set the body of the message to the `output application/json` value. The body is sent as a String as part of a JSON message, and needs to be serializable. Do not set the output as a Java object hashmap - the send appears to work, but the data cannot be converted back to a hashmap when read.
