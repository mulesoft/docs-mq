...
Anypoint MQ is an integration add-on to Anypoint Platform.
...
For more information about Anypoint Integration packages, see 
...
.1+| North America |            Canada (Central)       | `ca-central-1` <<mq-hyperforce,^***^>>
3+h| https://jp1.platform.mulesoft.com[Japan Cloud]
.1+| APAC |            Japan (Central)       | `ap-northeast-1` <<mq-hyperforce,^***^>>
|===
