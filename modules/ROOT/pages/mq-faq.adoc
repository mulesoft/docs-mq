...
Standard queues do not guarantee order for messages. The message order is not random, but the order is not enforced. If message order is important, use a xref:mq-faq.adoc#fifoqueues[FIFO queue] or set the Anypoint MQ connector to the <<process-one-by-one,Consume operation>>.
