...
If your app requires an additional layer of encryption security, then you can encrypt the payloads yourself before publishing messages to Anypoint MQ; however, you must manage your own encryption keys for both the message publishers and the consumers.
...
You can have the same object name for queues and exchanges between environments, but the Client ID and Client Secret must be different. It's not possible to access a production queue from a development environment if they don't have the corresponding app client IDs.
...
There is no built-in functionality to replay messages. Thus, you need to implement a special flow or app to consume those messages and forward them back to original queue for reprocessing.
...
There is no maximum transactions per second (TPS) for standard queues or exchanges. FIFO queues have a limit of 300 TPS; however, if you batch 10 messages per each read and write operation (maximum) using the API, FIFO queues can support up to 3,000 TPS.
