...
For batch-retrieve operations, use the `batchSize` query parameter to specify the number of messages to retrieve. Anypoint Broker API can't guarantee it will retrieve 10 messages in a single call as it depends on underlying system's polling mechanism and availability.
