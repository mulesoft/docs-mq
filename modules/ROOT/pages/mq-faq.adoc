...
Anypoint MQ non-FIFO and FIFO queues are available in these regions:
...
Standard (non-FIFO) queues do not guarantee order for messages. The message order is not random, but the order is not enforced. If message order is important, use a FIFO queue.
