...
Standard queues do not guarantee order for messages. The message order is not random, but the order is not enforced. If message order is important, use a FIFO queue.
...
To process messages one at a time, set the Anypoint MQ connector to the `consume` operation.
