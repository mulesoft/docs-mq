...
For batch-retrieve operations, use the `batchSize` query parameter to specify the number of messages to retrieve. Anypoint Broker API can't gurantee it will retrive 10 message a single call as it depends on underlying system polling mechanism and availability.
