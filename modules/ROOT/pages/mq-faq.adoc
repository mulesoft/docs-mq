...
.9+| US .4+| North America | Northern Virginia (`us-east-1`)
                  | Ohio (`us-east-2`)
                  | Oregon (`us-west-2`)
...
Anypoint MQ messages are always transported using in-flight HTTPS encryption with the latest Transport Layer Security (TLS) protocols. Anypoint MQ uses Transport Layer Security (TLS) 1.2 for secure transport and doesn't support the deprecated TLS versions 1.0 and 1.1.
//Shared Encryption
