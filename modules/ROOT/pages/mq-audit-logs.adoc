= Accessing Anypoint MQ Audit Logs
...
[NOTE]
--
The audit logging service doesn't record these actions as auditable:
...
image::mq-access-mgmt-audit-logs.png["Filters, Apply filters button, current filters, and Download payload button on the Audit logs page"]
...
For each action on the *Audit Log* page, click the *Download payload* icon (image:icon-download-payload.png["",2.5%,2.5%])
to download a payload file (`payload.txt`).

This example shows a formatted version of the
...
* xref:mq-apis.adoc[]
* https://dev-portal.mulesoft.com/apis/audit-log-query.html[Audit Log Query API^]
