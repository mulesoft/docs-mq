...
Binding::
+
A method for specifying a set of standard queues that form a message exchange, enabling apps send messages to multiple queues.
+
...
Duration::
The period that messages can persist in flight (for up to two weeks), after Anypoint MQ deletes them.
[[encrypt-queue]]
Encrypted Queue::
