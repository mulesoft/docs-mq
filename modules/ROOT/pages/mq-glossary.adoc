...
[[failover]]
Failover::
A feature that can be enabled for standard queues that assigns a fallback queue in a different region and automatically diverts message processing if an application can't reach the Anypoint MQ server in the primary region.
+
