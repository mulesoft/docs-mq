...
[[failover]]
Failover::
A feature that can be enabled for standard queues that assigns a <<fallback-queue,fallback queue>> in a different region and automatically diverts message processing if an application can't reach the Anypoint MQ server in the primary region.
+
For information, see xref:mq-failover.adoc[].

[[fallback-queue]]
Fallback Queue::
A queue that Anypoint MQ automatically creates in a <<fallback-region,fallback region>> when <<failover,failover>> is enabled for a standard queue.
...
The fallback region is managed by the Anypoint MQ server and is not configurable by end users.
+
For information, see xref:mq-failover.adoc[].
