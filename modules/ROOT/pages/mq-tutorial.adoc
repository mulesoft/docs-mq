...
[cols="5a,15a,50a"]
|===
| (*1*) | *Path* field | Enter `/mq/{messageId}`.
...
. In Exchange, click the *Provided by MuleSoft* navigation menu, select *Connectors* from the dropdown, and then search for Anypoint MQ Connector:
+
...
. Return to your *Anypoint Platform* > *MQ* browser window to copy the URL, client app ID, and client secret values:
+
.. Click *Destinations* and click *copy* to copy the URL to the *URL* field in Studio:
+
image::mq-tutorial-url.png["Copy button in the Destinations page"]
.. Click *Client Apps* and click `DemoClientApp` to display the details pane.
+
