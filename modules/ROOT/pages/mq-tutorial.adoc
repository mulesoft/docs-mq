...
. Create an app in Anypoint Studio 7, configure Anypoint MQ Connector, and test the connection.
...
== Create and Connect a Mule App to Anypoint MQ in Anypoint Studio 7

In this lesson, you will use Anypoint Studio 7 to create a Mule app similar to the app you created in Design Center. In Studio 7, you can deploy the app to CloudHub and submit a `curl` command to test the connection. 
...
. Select the version of the Anypoint MQ Connector to install.
+
The connector version depends on your Mule version (Mule 4 or Mule 3).
. Click *Add to project*.
+
.The arrow shows the *Add to project* icon in the Studio 7 toolbar.
...
+
.The screen shows (*1*) the *HTTP* and (*2*) *Listener* operations.
image::mq-tutorial-studio7-palette.png[HTTP and Listener operations in Mule Palette]
...
+
.The screen shows (*1*) the *Path* and (*2*) *Connector configuration* properties of the Listener.
...
. In the *Global Element Properties* window, verify that *Host* is set to `0.0.0.0` and *Port* is set to `8081`, and then click *OK*.
+
.The screen shows (*1*) the *Host* and (*2*) *Port* properties of the HTTP Listener.
image::mq-tutorial-studio7-http-config.png[Host and Port properties of the HTTP Listener]
+
