...
IMPORTANT: Disable password programs such as LastPass or Okta in your browser before using Design Center. Otherwise, the password program inserts passwords in the HTTP Listener's TLS tab. The rogue passwords in the TLS tab cause the app to fail. 
...
. From a command prompt or terminal, submit this curl command to get an MQ Broker API access
token:
+
[source,json]
----
...
----
+
You can get the values for CLIENT_ID and CLIENT_SECRET from your Anypoint Platform > MQ > Client Apps screen. 
+
The output of the curl command is:
+
[source,json]
----
{"access_token":"ACCESS_TOKEN","simple_client":{"envId":"ENVIRONMENT_ID","orgId":"ORGANIZATION_ID"},"token_type":"bearer"}
----
+
. Put the ACCESS_TOKEN value into this command and run this command from a command prompt to send the "body" value as an MQ message:
+
[source,json,linenums]
----
curl -X POST \
...
. Search for the Mule Anypoint MQ Connector, click *Add to project*, and click Proceed.
...
+
. In a command prompt or terminal, get an access token by replacing CLIENT_ID and CLIENT_SECRET 
with the values from Anypoint Platform > MQ:
+
[source,json]
----
$ curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "client_id=CLIENT_ID&client_secret=CLIENT_SECRET&grant_type=client_credentials" "https://mq-us-east-1.anypoint.mulesoft.com/api/v1/authorize"
----
+
. Put the ACCESS_TOKEN value into this command and run this command from a command prompt to send the "body" value as an MQ message, and replace DEPLOYMENT_URL with the *App url* value:
+
[source,json,linenums]
----
curl -X POST \
