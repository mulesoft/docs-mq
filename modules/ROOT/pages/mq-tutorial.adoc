...
+
image::mq-tutorial-get-messages.png[Message Browser option (1) and Get Messages button (2)]
. Specify the number of messages (1-500) and the time to poll (1-3600 seconds).
. Click *Get Messages*.
. Click the message row (*1*) to view the message payload (*2*) in the details pane:
+
...
+
* Click *Full Screen* (*5*) to display the message payload in a separate window.
* Click *X* (*6*) to close the details pane.
* Click *Delete* (*4*) to delete the selected message.
+
...
* Click *Return Messages* (*3*) to return all messages to the queue (for example, to enable other applications to read the messages). 
+
...
. Click the blue plus (+) icon (*2*).
. In the *Create Client App* window, enter `DemoClientApp` and click *Save Changes*.
. Click the client app entry to view the client ID (*1*) and client secret value (*2*) in the details pane:
+
...
. Keep the *Client Apps* window open so you can copy (*3*) the client app ID and client secret in a later step.
...
. Click *HTTP Listener*.
. In the *HTTP Listener* window, set the value of *Path* (*1*) to `/mq/{messageId}` and then click the *Close* (*2*) button (X):
+
...
. Return to your *Anypoint Platform* > *MQ* browser window to copy the client app ID and client secret:
+
.. Click *Destinations* and click *copy* to copy the URL and paste the value in the *URL* field in the *Design Center* page.
+
...
+
image::mq-tutorial-client-apps-detail.png[Client App ID field, Client Secret field, and Copy buttons] 
.. In the *MQ* details pane, click the *Copy* button for the *Client App ID* field (*1*) and paste the value in the *Client App ID* field in the *Design Center* page.
.. In the *MQ* details pane, click the *Copy* button for the *Client Secret* field (*2*) and paste the value in the *Client Secret* field in the *Design Center* page:
+
image::mq-tutorial-dc-client-id-secret.png[Client App ID field and Client Secret field in the Design Center page]
...
. Click *Message Browser* (*1*) in the left menu:
+
image::mq-tutorial-get-messages.png[Message Browser option and Get Messages button]
. Specify the number of messages (1-500) and the time to poll (1-3600 seconds).
. Click *Get Messages* (*2*).
+
You see the messages in the queue:
+
...
. Click the message row (*1*) to view the message payload (*2*) that you sent in the `curl` command in the details pane.
* Click *Full Screen* (*5*) to display the message payload in a separate window or *X* (*6*) to close the details pane.
* Click *Delete* (*4*) to delete the selected message.
+
//Shared Delete Message
include::partial$mq-delete-msg.adoc[tag=deleteMsg]
* Click *Return Messages* (*3*) to return the message to the queue (for example, to enable other applications to read the message). 
+
[NOTE]
//Shared Big Payload
include::partial$mq-big-payload.adoc[tag=mqBigPayload]
...
. In the Listener properties window, set *General* > *Path* (*1) to `/mq/{messageId}`:
+
image::mq-tutorial-studio7-listener-props.png[Path property and Connector configuration icon for the Listener]
. Click the green plus (+) icon (*2*) to the right of the *Connector configuration* field.
...
. In the Publish properties window, set *Destination* (*1*) to the `myDemoQueue` queue that you created earlier in *Anypoint Platform* > *MQ*:
+
...
.. Click the *Copy* button for the *Client App ID* field (*1*) and paste the value in the *Client App ID* field in the *Anypoint MQ Config* window.
.. Click the *Copy* button for the *Client Secret* field (*2*) and paste the value in the *Client Secret* field in *Anypoint MQ Config* window:
+
...
. Click *Test Connection* (*4*) to ensure that the values are correct, and then click *OK* twice.
. Save your project. 
