...
+
include::partial$mq-versions.adoc[tag=mqConnectorVersion]
...
+
Depending on your Mule version, you create your app in Studio 7 (Mule 4.x) or Studio 6 (.9).
...
. Enter `5` for the the number of messages to retrieve and `10` for the number of seconds to poll.
+
...
. Keep the *Connected Apps* page open so you can copy the ID and secret in a later step.
...
.. Return to the <<connected-apps-window,Connected Apps>> or <<client-app-window,Client Apps>> window and copy the ID and secret for the app.
...
+
This is the <<create-demo-queue,name>> of the queue that you created earlier in this tutorial.
. Click the *Close* (*X*) icon to close the *Publish Configuration* page:
+
image::mq-tutorial-dc-mq-close.png["Close icon"]
. Select *Deploy Application* from the *V* menu: 
+
...
. Click the message row to view the message payload in the details pane:
+
image::mq-tutorial-msg-browser-payload-details-nocallouts.png["Message payload in the details pane"]
...
== Create and Connect a Mule 3.9 App to Anypoint MQ in Anypoint Studio 6
...
This section applies only to Anypoint Studio 6 and Mule 3.9. For Studio 7, see <<Create and Connect a Mule 4 App to Anypoint MQ in Anypoint Studio 7>>.
...
.. In the *Anypoint MQ Config* window in Studio, paste the URL in the *URL* field.
.. Return to the <<client-app-window,Client Apps>> window and copy the client app ID and client secret for the app.
.. Paste the client app ID and client secret values to the *Anypoint MQ Configuration* window in Studio:
+
