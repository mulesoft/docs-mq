...
.. Enter your Anypoint Platform username and password.
.. In Exchange, select *Provided by MuleSoft* (*1*), and then search for Anypoint MQ Connector:
+
...
. In the HTTP properties window, set *General* > *Path* (*1*) to `/mq/{messageId}`:
+
image::mq-tutorial-studio6-http-props.png[HTTP properties]
. Click the HTTP connector and click the green plus icon (+) to the right of *Connector Configuration* (*2*).
...
. Copy the following information from Anypoint MQ to the Anypoint MQ Configuration *Global Element Properties* window:
+
.. In Anypoint Platform, select *MQ* > *Destinations*.
.. Click *Copy* to copy the URL and paste it in the *URL* field in Studio:
+
image::mq-tutorial-url.png[Copy button on the Destinations page]
.. Click *Client Apps* (*1*) and click `DemoClientApp` to display the details pane:
+
image::mq-tutorial-client-apps-detail.png[Client App ID field, Copy, and Show buttons]
.. Click the *Copy* button (*2*) for the *Client App ID* field and paste the value in the *Client App ID* field in the *Anypoint MQ Configuration* window.
.. Click the *Copy* button  (*3*) for the *Client Secret* field and paste the value in the *Client Secret* field in *Anypoint MQ Configuration* window:
+
...
. Click *OK*.
. In the Anypoint MQ properties window, click the *Operation* field and select *publish* (*1*) from the menu.
. Set *Destination* (*2*) to the `myDemoQueue` queue that you created earlier in *Anypoint Platform* > *MQ*. 
+
The queue must be in an environment other than Design.
+
