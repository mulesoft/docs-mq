...
. Depending on your authentication method, either:
** In Access Management, configure a Connected App and use the ID and secret to configure Anypoint MQ Connector. (Recommended)
** In Anypoint MQ, register a client app and use the client ID and client secret use to configure Anypoint MQ Connector. 
...
. Click *MQ* in the navigation menu.
...
.. Blue plus (*+*) icon creates a new queue or exchange.
...
. Enter `3` for the the number of messages to retrieve and `10` for the number of seconds to poll.
+
...
+
image::mq-tutorial-msg-browser-payload-details.png["Selected message row, payload in the details pane, and icons representing Return Messages, Delete, Full Screen, and close details pane (X)"]
+
include::partial$mq-msg-browser-details.adoc[tag=mqMsgBrowserDetails]


You have successfully created a queue in Anypoint MQ, sent messages to the queue, verified that the messages arrived, and viewed the message payload. Next, you create a Connected App or client app to connect a Mule app to Anypoint MQ.


== Create a Connected App or Client App

After you test the queue, you configure an app, which generates the ID and secret you use to configure Anypoint MQ Connector.

Depending on the authentication method, either:

* <<create-connected-app,Create a Connected App>> and use the ID and secret to configure Anypoint MQ Connector. (Recommended)
* <<create-client-app,Create a client app>> and use the client ID and client secret use to configure Anypoint MQ Connector. 

[[create-connected-app]]
=== Create a Connected App

include::partial$mq-shared-connect-apps.adoc[tags=!ScopesConnApp]
. [[connected-app-window]] Keep the *Connected Apps* page open so you can copy the ID and secret in a later step.
...
include::partial$mq-shared-client-apps.adoc[tags=mqRegisterClientApp;!excludeClientAppTutorial]
. [[client-app-window]] Keep the *Client Apps* window open so you can copy the client app ID and client secret in a later step.

You have successfully registered a client app in Anypoint MQ. Next, you create a Mule app in Design Center and connect it to Anypoint MQ.


[[configure-design-center]]
== Create and Connect a Mule App to Anypoint MQ in Design Center

After you configure the Connected App or client app, you use Design Center to configure an app that connects to Anypoint MQ, submit REST API commands to the Anypoint MQ Broker API, and use Anypoint MQ to view the results.
...
* Leave the *Connected Apps* or *Client Apps* window open so that you can access the queue and the ID and secret values for the Connected App or client app.
...
. Click *HTTP Listener*.
. In the *HTTP Listener* window, set the value of *Path* to `/mq/{messageId}` and then click the *Close* (*X*) icon:
+
image::mq-tutorial-dc-http-path.png["Path value and Close button"]
+
[calloutlist]
.. *Path* field specifies the path to the message ID.
.. *Close* (*X*) icon closes the *HTTP Listener* window.
...
.. Click *Copy* and select *Connector version 4.x and later* from the menu to copy the URL:
+
...
.. Return to the <<connected-app-window,*Connected Apps*>> or <<client-app-window,*Client Apps*>> window.

Click *Client Apps* and click `DemoClientApp` to display the details pane:
+
image::mq-tutorial-client-apps-detail.png["Client app name and Copy and Show buttons"]
+
...
[NOTE]
include::partial$mq-connector-share.adoc[tag=mqUrlVersion]
--
...
. Click the *Close* (*X*) icon to close the *Publish Configuration* page:
+
...
. Click the name of the `myDemoQueue` queue that you created earlier and specified in the *Destination* setting for your app in Design Center.
. Click *Message Browser* navigation menu:
+
...
image::mq-tutorial-url.png["Copy button in the Destinations page"]
+
include::partial$mq-connector-share.adoc[tag=mqConnectorVersion]
...
image::mq-tutorial-url.png["Copy button in the Destinations page"]
+
Use Anypoint MQ Connector version 3.9 with Mule 3 and Studio 6. 
.. In the *Anypoint MQ Config* window in Studio, paste the URL in the *URL* field.
.. Click *Client Apps* and click `DemoClientApp` to display the details pane.
+
image::mq-tutorial-client-apps-detail.png["Client app name and Copy and Show buttons"]
