...
| (*2*) | Blue plus (*+*) icon | Creates a new queue or exchange.
|===
...
+
Queue names can contain up to 127 alphanumeric characters (a-z, A-Z, 0-9) and punctuation (. -). They cannot contain spaces or other characters. 
...
[cols="5a,30a,50a"]
|===
| (*1*) | *Client Apps* navigation menu | Displays and creates client apps.
| (*2*) | Blue plus (+) icon| Adds a client app.
|===
...
[cols="5a,15a,50a"]
|===
| (*1*) | *Path* field | Specifies the path to the message ID.
| (*2*) | *Close* (*X*) icon| Closes the *HTTP Listener* window.
|===
