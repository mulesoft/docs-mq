...
. Register a Mule app in Anypoint MQ, which you then use to configure Anypoint MQ Connector. 
...
+
Depending on your Mule version, you will create your app in Studio 7 (Mule 4) or Studio 6 (Mule 3).
...
In this lesson, you will create a queue, send messages to the queue, verify that the messages arrived, and view the message payload.
...
. Click *Destinations*:
+
.The screen  shows (*1*) the *Destinations* option and (*2*) the blue plus icon (+).
...
+
.The *Destinations* page shows (*1*) the ID, (*2*) type, and (*3*) details label for the queue.
...
You have successfully created a queue. Next, you will send messages to the queue, verify that the messages arrived, and view the message payload.

== Send Messages to a Queue and Verify That They Arrived

In this lesson, you will test the queue you created by sending messages to it, validating that the messages arrived in the queue, and retrieving them from the queue.
...
. Click *Send*.
+
Next, you will verify that the messages arrived in the queue.
. Click *Destinations*.
...
+
.The screen shows (*1*) the *Message Browser* option and (*2*) the *Get Messages* button.
...
+
.The screen  shows (*1*) the message row, (*2*) the message payload in the details pane, (*3*) the *Return Messages* icon, and (*4*) the trash can icon.
image::mq-tutorial-msg-browser-payload-details.png[Selected message row, payload in the details pane, and *Return Messages* and trash can icons]
...
You have successfully created a queue in Anypoint MQ, sent messages to the queue, verified that the messages arrived, and viewed the message payload. Next, you will register a client Mule app with Anypoint MQ.


== Register a Client Mule App

In this lesson, you will register a client Mule app in Anypoint MQ, which generates the client app ID and client secret for the app that you need to configure Anypoint MQ Connector.
...
. Click *Client Apps*:
+
.The screen  shows (*1*) the *Client Apps* option and (*2*) blue plus icon (+) icon.
...
+
.The screen shows (*1*) the *Client App ID* field, and (*2*) the *Show* and (*3*) *Copy* buttons.  
...
You have successfully registered a client Mule app in Anypoint MQ. Next, you will create a Mule app in Design Center and connect it to Anypoint MQ.


== Create and Connect a Mule App to Anypoint MQ in Design Center

In this lesson, you will use Design Center to configure an app that connects to Anypoint MQ, submit REST API commands to the Anypoint MQ Broker API, and use Anypoint MQ to view the results.
...
Before configuring an app in Design Center, leave a browser window open to *Anypoint Platform* > *MQ* so that you can access the queue and the client app ID and client secret values for the client app you registered in the previous lesson.
...
+
. Click *HTTP Listener*.
. In the HTTP Listener window, set the value of *Path* to `/mq/{messageId}`, then click the *Close* button (X).
+
.The screen shows (*1*) the *Path* value and (*2*) the *Close* button (X).
...
+
.The screen shows (*1*) the *Destination* field and (*2*) the *Click here to set it up* link.
...
+
.The screen shows (*1*) the *Client App ID* field, (*2*) *Show* button, and (*3*) *Copy* button in the *Client Apps* details pane.  
image::mq-tutorial-client-apps-detail.png[Client App ID field, Show and Copy buttons]
+
...
+
.The screen shows (*1*) the *Client App ID* field and (*2*) *Client Secret* field in the *Design Center* page.  
image::mq-tutorial-dc-client-id-secret.png[Client App ID field and Client Secret field]
+
. Click *Test* to ensure that you have access to your Mule app.
+
...
You have successfully used Design Center to configure an app that connects to Anypoint MQ. Next, you will test the connection by submitting REST API commands to the Anypoint MQ Broker API and using Anypoint MQ to view the results.

=== Test Your Design Center App

In this lesson, you will test your app using a `curl` command to submit a REST API command to the Anypoint MQ Broker API and use Anypoint MQ to view the results.
...
. Run this command from a command prompt to send the `body` value as an Anypoint MQ message. 
+
...
+
+
Next, you will verify that the message arrived in the queue.
...
+
.The screen shows (*1*) the *Message Browser* option and (*2*) the *Get Messages* button for getting messages from the `myDemoQueue` queue.
image::mq-tutorial-get-messages.png[Message Browser option and Get Messages button]
. Click *Get Messages*.
. Click the message row to view the message payload that you sent in the `curl` command in the details pane:
+
.The screen shows (*1*) the message row and (*2*) the message payload in the details pane.
image::mq-tutorial-msg-browser-payload-details-curl.png[Selected message row and payload in the details pane]
+
. Try changing the body text in the `curl` command and repeat the steps to view the message.

You have successfully tested your app using a `curl` command to submit a REST API command to the Anypoint MQ Broker API and using Anypoint MQ to view the results. Next, you will create a Mule app in Anypoint Studio 7 and connect it to Anypoint MQ.
...
In this lesson, you will use Anypoint Studio 7 to create a Mule 4 app similar to the app you created in Design Center. In Studio 7, you can deploy the app to CloudHub and submit a `curl` command to test the connection. 
...
In this lesson, you will use Studio 7 to configure an app that connects to Anypoint MQ.
...
+
.The screen shows (*1*) the *Provided by MuleSoft* option and (*2*) the search field.
...
. At the confirmation prompt, click *Proceed*, then click *OK*.
. In the *Mule Palette*, click *HTTP*, and then drag the *Listener* to the Studio canvas.
+
.The screen shows (*1*) the HTTP Connector and (*2*) the Listener operation.
...
+
.The screen shows (*1*) the *Path* property and (*2*) the *Connector configuration* icon for the Listener.
...
+
.The screen shows (*1*) the *Destination* property of the Publish operation and (*2*) the green plus icon (+) for connector configuration.
...
+
.The screen shows (*1*) the *URL* field, (*2*) the *Client App ID* field, (*3*) the *Client Secret* field, and (*4*) the *Test Connection* button.  
...
+
. Click *Test Connection* to ensure the values are correct, and then click *OK* twice.
. Save your project. 
...
You have successfully used Studio 7 to configure an app that connects to Anypoint MQ. Next, you will test the connection by submitting REST API commands to the Anypoint MQ Broker API and using Anypoint MQ to view the results.
...
+
To redeploy your Mule app to CloudHub at any time, right-click the project in Studio 7 and select *Anypoint Platform* > *Deploy to CloudHub*. 
...
+
.The screen shows (*1*) the message row and (*2*) the message payload in the details pane.
image::mq-tutorial-msg-browser-payload-details-curl-rtm.png[Selected message row and payload in the details pane]
+
. Try changing the body text in the `curl` command and repeat the steps to view the message.
...
In this lesson, you will use Anypoint Studio 6 to create a Mule 3 app similar to the app you created in Design Center. In Studio 6, you can deploy the app to CloudHub and submit a `curl` command to test the connection. 
...
image::mq-tutorial-studio6-flow-complete.png[Flow in Studio 7 Canvas]
. In the canvas, click *HTTP*.
. In the HTTP properties window, set *General* > *Path* to `/mq/{messageId}`:
+
.The screen shows (*1*) the *Path* property and (*2*) the *Connector configuration* icon for the HTTP Connector.
...
+
.The screen shows (*1*) the *URL* field, (*2*) the *Client App ID* field, and (*3*) the *Client Secret* field.  
...
The queue must be in an environment other than Design.
+
.The screen shows (*1*) the *Operation* and *Destination* properties in the Anypoint MQ properties window.
...
You have successfully used Studio 6 to configure a Mule 3 app that connects to Anypoint MQ. Next, you will test the connection by submitting REST API commands to the Anypoint MQ Broker API and using Anypoint MQ to view the results.
