...
+
. In the following command, replace DEPLOYMENT_URL with the *App url* value from Runtime Manager. Run this command from a command prompt to send the "body" value as an MQ message. You no longer need the authorization header that was required to access the Design Center application. Runtime Manager takes care of authorization for you.
+
[source,json,linenums]
----
curl -X POST \
