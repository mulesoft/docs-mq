...
https://anypoint.mulesoft.com/#/signin[Anypoint Platform] that
...
*Note:* The MQ connector description in this tutorial is for Studio 6 and Mule 3. However the MQ user interface descriptions
work for both Mule 3 and Mule 4.

== Prerequisites

. Read xref:mq-understanding.adoc[Understanding MQ].
...
. If your organization previously created a dead letter queue where undeliverable messages go, click *Assign a Dead Letter Queue* to enable this option. For more information on dead letter queues, see xref:mq-queues.adoc#create-a-queue[Creating a Queue].
...
. Follow the directions in <<Sending a Message to a Queue>> and
...
+
Alternatively, you can delete the message by clicking the trash can icon. Anypoint MQ provides three modes for how messages delete after they are viewed. For more information, see xref:mq-ack-mode.adoc[Acknowledging Messages].
+
...
== Configuring Studio for the Anypoint MQ Connector Plugin
...
== Running the Studio Application
...
== Binding a Queue to a Message Exchange
...
== Sending a Message to a Message Exchange
...
in <<Getting a Message From a Queue>>.

You can also send comma-separated value (CSV) or JSON content in the payload by changing
the message Type value. For more information, see <<Sending a CSV or JSON Message>>.
