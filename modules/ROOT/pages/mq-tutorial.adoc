...
+
. Click the blue plus icon image:mq-blue-plus-icon.png[].
. Specify an app name such as `DemoClientApp` and click *Save Changes*.
+
...
Before starting to configure an app in Design Center, leave a browser window open to *Anypoint 
Platform* > *MQ*. Determine the queue you want to access and have a Mule app available so you can copy its client ID and secret.
...
+
For the other operations, if you were constructing your Mule app to receive a message sent from a queue, you would use the *Consume* operation. If your app had tests to determine if message processing succeeded, you would use the *Ack* operation  
...
window, click *Client Apps* and click the name of an app. Click *Copy* for each ID and 
secret values and paste each in the Design Center window.
+
image::mq-dc-client-id-secret.png[]
+
. Click *Test* to ensure that you have access to your Mule app. If successful,
...
----
+
You can find the value for `DEPLOYMENT_URL` in the detail for your app in the Design Center window:
+
...
You can use Anypoint Studio 7 to create a Mule app that uses Anypoint MQ much the same way as you
created in Design Center. Studio 7 enables you to create the Mule app, 
you can test the program, and then package the Mule app into a zip file to 
put into *Anypoint Platform* > *Runtime Manager*. Runtime Manager serves the Mule app so
you can submit `curl` commands as you did in the previous <<Configure Anypoint MQ in Design Center>> section. See xref:mq-connectors.adoc[Anypoint MQ Connectors] for more information.
...
+
If you see this message: `You do not have permission to deploy new applications in this environment. Please select an existing app to redeploy`, change the environment so that it is *not* the Design environment. For example, set the environment to Production or Sandbox.
To change the environment, click *Design* in the upper left corner, and select another environment.
+
You may also need to change the name of the app to deploy it successfully.
+
. Browse to *Anypoint Platform* > *Runtime Manager*. Ensure the app is running. Click
the app and copy the *App url* value:
+
...
+
You can redeploy your Mule app by right-clicking the project in Studio 7 and 
...
The following view shows the Mule app in Package Explorer, the position of the HTTP Listener and
...
You can test your app using the same curl command as in 
<<Create a Studio 7 Mule Application>>. Deploy your Mule app from 
Studio to CloudHub and after the app starts in Runtime Manager, you can
...
. Log in to *Anypoint Platform* > *MQ*.
. Click *Destinations*.
. Click the blue plus icon image:mq-blue-plus-icon.png[]
. Click *Exchange*.
