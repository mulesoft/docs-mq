...
+
Anypoint MQ isn't available in the trial edition of Anypoint Platform.
* Anypoint MQ integration add-on
+
Contact your account team to purchase and activate Anypoint MQ.
...
+
image::mq-switch-env.png["Destinations page showing the environment-selection button"]
. Click *Destinations*:
+
image::mq-tutorial-destinations-option.png["Destinations page showing the Destinations navigation option and the Add button for creating a new queue"]
+
[calloutlist]
.. *Destinations* navigation menu displays available queues.
.. *Add* icon creates a new queue or exchange.
. Click the *Add* icon (image:icon-blue-plus.png["",2.5%,2.5%]) to display the menu:
+
image::mq-tutorial-create-choices.png["Add icon showing the Queue, Exchange, and FIFO Queue menu options"]
. Select *Queue*.
. [[create-demo-queue]] In the *Create Queue* window, enter `myDemoQueue` in the *ID* field:
+
image::mq-create-queues.png["Create Queue window showing myDemoQueue in the ID field and other default options"]
// replace screenshot
+
Queue names can contain up to 127 alphanumeric (a-z, A-Z, 0-9) and punctuation (. -) characters. They can't contain spaces or other characters. 
...
+
image::mq-tutorial-queue-details.png["Destinations page showing the new queue"]
...
+
image::mq-tutorial-message-sender-page.png["Message Sender page and option in the navigation menu"]
...
+
image::mq-tutorial-message-sender-user-props.png["Message Sender page showing the Add User Properties toggle"]
+
include::partial$mq-user-props.adoc[tag=mquserpropsTypeColor]
. Click *Send*.
. Set the *Type* to *CSV*, enter this in the *Payload* field, and click *Send*:
+
[source,csv,linenums]
----
"dog",
...
"parrot"
----
. Set the *Type* to *JSON*, enter this in the *Payload* field, and click *Send*:
+
...
+
image::mq-tutorial-msgs-in-queue.png["Details pane showing three messages in the myDemoQueue queue"]
. In the details pane, click the queue details label `myDemoQueue`.
. Click *Message Browser* in the navigation menu:
+
image::mq-tutorial-get-messages.png["Message Browser page and option in the navigation menu"]
...
+
image::mq-tutorial-msg-browser-payload-details.png["Selected message row and details pane showing the payload"]
+
[calloutlist]
.. Selected message
...
+
image::mq-tutorial-exchange.png["Exchange window showing the Provided by MuleSoft menu option and Connectors selected, and "anypoint mq" in the search field"]
. Select *Anypoint MQ Connector - Mule 4*.
+
Studio 7.x supports Mule 4.x.
. Click *Add to project*:
+
image::mq-tutorial-exchange-add-to-project.png["Add to project option in Exchange"]
...
+
image::mq-tutorial-studio7-listener-props.png["Listener properties window showing the Path field value"]
. Click the green plus icon (image:icon-green-plus.png["",2.5%,2.5%]) for the *Connector configuration* field.
...
+
image::mq-tutorial-studio7-mq-props.png["Destination property for the Publish operation in Anypoint Studio"]
+
The location of the queue must be an environment other than Design.
. Click the green plus icon (image:icon-green-plus.png["",2.5%,2.5%]) for the *Connector configuration* field to display the *Global Element Properties* window.
...
+
image::mq-tutorial-url.png["Anypoint MQ Destinations page showing the Copy button and the Connector version dropdown"]
+
include::partial$mq-versions.adoc[tags=mqConnectorVersion3x]
...
. In the Runtime Manager *Deploying Application* window, click *Deploy Application*:
+
image::mq-tutorial-deploy-app.png["Deploying Applications page in Runtime Manager showing the Deploy Application button"]
...
+
image::mq-tutorial-rtm-deploy-url.png["Runtime Manager application details pane showing the App url value"]
. Run this `curl` command from a command prompt to send the `body` value as an Anypoint MQ message.
+
...
. Click *Message Browser* in the navigation menu:
+
image::mq-tutorial-get-messages.png["Message Browser page and navigation menu option"]
. Enter `5` for the number of messages to retrieve and `10` for the number of seconds to poll.
. Click *Get Messages*.
+
Anypoint MQ retrieves the messages from the queue and displays them in the Message Browser.
...
Click the *Configuration XML* tab in Studio and compare your XML to this example.
...
+
image::mq-tutorial-studio6-flow-complete.png["Flow in Studio 6.x Canvas showing the Logger"]
. In the canvas, click *HTTP*.
. In the HTTP properties window, set *General* > *Path* to `/mq/{messageId}`:
+
image::mq-tutorial-studio6-http-props.png["HTTP properties window showing the Path field value"]
. Click the HTTP connector and click the green plus icon (image:icon-green-plus.png["",2.5%,2.5%]) for *Connector Configuration*.
...
----
+
image::mq-tutorial-studio6-set-payload-props.png["Set Payload window showing the payload in the Value field"]
. In the canvas, click *Anypoint MQ*.
. In the Anypoint MQ properties window, click the green plus icon (image:icon-green-plus.png["",2.5%,2.5%]) for *Connector Configuration*.
...
+
image::mq-tutorial-studio6-mq-config.png["Anypoint MQ Config window showing the URL, Client App ID, and Client Secret fields"]
. Click *OK*.
. In the Anypoint MQ properties window, click the *Operation* field and select *publish* from the menu:
+
image::mq-tutorial-studio6-mq-props.png["Anypoint MQ properties window showing the Operation and Destination fields"]
...
+
image::mq-tutorial-studio6-logger-props.png["Logger properties window showing the payload in the Message field"]
+
. Save your project.
...
To test your Mule 3.9 app, follow the steps in <<test-app-in-studio>> to use a `curl` command to submit a REST API command to the Anypoint MQ Broker API.
Then, use Anypoint MQ to view the results.
