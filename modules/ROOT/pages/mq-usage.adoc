...
NOTE: Enabling failover for a queue can result in increased API requests because Anypoint MQ periodically checks the fallback queue for messages to consume. 
For more information, see xref:mq-failover.adoc#perf-and-billing[Performance and Billing Considerations].
