...
NOTE: Enabling failover for a queue can result in increased message requests because Anypoint MQ checks the fallback queue for messages to consume. 
For more information, see xref:mq-failover.adoc[].
// need an anchor target here.
