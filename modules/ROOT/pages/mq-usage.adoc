...
NOTE: Enabling failover for a queue can result in increased API requests because Anypoint MQ Connector periodically checks the fallback queue for messages to consume. 
For more information, see xref:mq-failover.adoc#configure-fallback-frequency[Configure Frequency of Fallback Queue Checks].
