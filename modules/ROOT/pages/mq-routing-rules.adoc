...
For information on using the Anypoint Admin API to manage message routing rules, see
...
* Property values for routing rules can contain alphanumeric characters (a-z, A-Z, 0-9), parentheses (()), at sign (@), exclamation point (!), ampersand (&), pipe (|), hash (#), dollar sign ($), percent (%), whitespace, hyphen (-), underscore (_), colon (;), semicolon (:), comma (,), period (.), and single-quote (') characters.
