...
* *Encrypted queues*
+
Queue data can optionally be encrypted, enabling companies to be compliant with their data at rest policies. Anypoint MQ messages are encrypted server-side.
...
* Lock ID - A lock that lets an app read a queue exclusively. Anypoint MQ provides the lock when an app acks a message from a queue. A locked message does not block other messages to be read. The lock makes a message unavailable to other applications while locked.
...
* Point-to-point - A pattern that allows an app to send a message that only a single app can receive.
