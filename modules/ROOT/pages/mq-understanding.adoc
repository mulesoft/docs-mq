...
Anypoint MQ supports hybrid use cases, IoT where you collect data from different devices, and a REST API you can use with device applications. You can also use Anypoint MQ with other app frameworks such as node.js to communicate to queues.
...
* *Delay queues*
+
With delay queues, you can postpone delivery of new messages to a queue for a specified period of time. Consumers can't see messages sent to a delay queue for the duration of the delay period. The minimum delay for a queue is 0 milliseconds. The maximum is 15 minutes.
+ 
...
+
For messages in a standard queue, you can postpone delivery of a particular message to the queue for a specified period of time. Consumers can't see messages sent with delivery delay for the duration of the delay period. The minimum delivery delay for a message is 0 milliseconds. The maximum is 15 minutes.
+ 
...
If enabled, Anypoint MQ encrypts messages using PBE with MD5 and Triple DES, and with a 168-bit key.
Anypoint MQ messages are encrypted server-side.

== Queues

Message queues provide temporary storage that enable applications to communicate in a scalable and reliable way. Publishers can publish messages into queues, and subscribers can receive messages from queues. You can create a dead letter queue to store undeliverable messages.
...
+
By putting a message on a queue, the work to process that message can be distributed across a number of different subscribers.
* *Reliability*
+
...
+
There is no limit on the number of messages in a queue. Each messages in a queue is limited to 10 MB. 
* *Long polling*
+
...
+
Billing is based on the quantity of messages delivered to subscribers from queues, not the number of queues or message exchanges you create.
...
* *In Flight*
+
Messages awaiting ack or nack, or with an expired lock time-to-live setting. In flight messages are not visible to other consumers. Anypoint MQ supports up to 120,000 in flight messages per queue for non-FIFO queues. FIFO queues can have up to 10 in flight messages per queue.
* *Duration*
+
...
* *Lock ID*
+
A lock that lets an app read a queue exclusively. Anypoint MQ provides the lock when an app acks a message from a queue. A locked message does not block other messages to be read. The lock makes a message unavailable to other applications while locked.
* *Message*
+
...
* *Usage*
+
A chart the illustrates how much data and API requests have been consumed.
