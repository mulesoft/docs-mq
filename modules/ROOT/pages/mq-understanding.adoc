...
* *In Flight*
+
Messages awaiting ACK or NACK, or with an expired lock time-to-live setting. In-flight messages are not visible to other consumers. Anypoint MQ supports up to 120,000 in-flight messages per queue for standard (non-FIFO) queues. FIFO queues can have up to 10 in-flight messages per queue.
* *Duration*
+
