...
How long a message waits before being put back into the queue in the event of server failure when the message is not acknowledged.
...
* *In Flight*
+
Messages that have been removed from the queue, have not yet been acknowledged or not acknowledged, and have not been returned to the queue due to an expired <<message_lock_ttl,message lock TTL>>. In-flight messages are not visible to other consumers. 
+
Anypoint MQ supports up to 120,000 in-flight messages per queue for standard (non-FIFO) queues. FIFO queues can have up to 10 in-flight messages per queue.
