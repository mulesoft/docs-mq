...
By default, a message exchange sends each message to every bound queue.
You can instead route messages to specific bound queues
...
Anypoint MQ forwards messages published to a message exchange to the bound queues
that are eligible under the binding configuration, and the app consumes from the queue.
...
Sending a message to a message exchange is similar to sending a message to a queue.
The exchange evaluates its bindings and sends the message to each eligible bound queue.
The maximum size for a message sent to a message exchange is 10 MB.

[IMPORTANT]
include::partial$mq-msg-exchange.adoc[tag=msgPublishRoutingOutcome]

For details about eligible bindings and unmatched messages, see xref:mq-routing-rules.adoc[].
...
+
image::mq-exchange-payload.png["Message Sender option for the selected message exchange"]
+
