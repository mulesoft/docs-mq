...
Before creating a message exchange, create one or more queues. For more information, see xref:mq-queues.adoc#to-create-a-queue[To Create a Queue].
...
in xref:mq-queues.adoc#to-get-a-message-from-a-queue[To Get a Message From a Queue].

You can also send comma-separated value (CSV) or JSON content in the payload by changing
the message *Type* value. For more information, see xref:mq-queues.adoc#to-send-a-csv-or-json-message[To Send a CSV or JSON Message].
