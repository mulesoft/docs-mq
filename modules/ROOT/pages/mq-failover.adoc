...
See <<region-unavailability>> for information on what can cause the region to be considered unavailable.
...
While the primary region is down, the consumer checks the fallback queue for new messages.
...
+
If you previously enabled failover for the queue, the fallback queue already exists.
.. Click *Destinations*.
...
NOTE: Disabling failover for a queue doesn't delete the fallback queue.
