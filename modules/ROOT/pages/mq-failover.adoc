...
+
Publishing to the fallback queue is slower than publishing to the primary queue due to the checks and the expected network latency of processing from a queue in a different region.
+
