...
+
image::mq-blue-create.png["Destinations page showing the Add icon for adding a queue"]
.. Select *Queue*.
+
...
+
image::mq-queue-details-primary-queue.png["Details pane for the primary queue showing the fallback queue and region"]
+
[calloutlist]
...
+
image::mq-queue-details-fallback-queue.png["Details pane for the fallback queue showing the primary queue and region"]
+
[calloutlist]
...
+
image::mq-queue-settings-failover-disabled.png["Queue Settings page showing the failover option deselected"]
. Click *Save Changes*.
+
...
.2+| Europe        | Europe (Ireland)**     |`eu-west-1`| UK (London)|`eu-west-2`
                   | UK (London)**      |`eu-west-2`| Europe (Ireland)|`eu-west-1`
