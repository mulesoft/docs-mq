...
* Authentication
+
If the authentication server is down when the connector switches to the fallback region, the connector might not be able to successfully switch to the fallback region if the fallback region can't validate the access token.
* Anypoint MQ UI
+
...
.4+| North America | US East (N. Virginia)|`us-east-1` | US East (Ohio)|`us-east-2`
                   ...
                   | <<region-note,Canada  (Central)>> *    |`ca-central-1`| US East (N. Virginia)|`us-east-1`
...
.3+| APAC          | Asia Pacific (Singapore) |`ap-southeast-1`| Asia Pacific (Tokyo)|`ap-northeast-1`
                   | <<region-note,Asia Pacific (Sydney)>> * |`ap-southeast-2`| Asia Pacific (Singapore)|`ap-southeast-1`
