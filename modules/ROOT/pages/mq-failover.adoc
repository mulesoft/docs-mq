...
and the access tokens are cached, processing continues in the fallback region.
If the access tokens aren't cached, Anypoint MQ won't be able to validate access tokens and processing won't continue in the fallback region. 
* Anypoint MQ UI
+
