...
+
.The arrow shows the accept checkbox for getting messages from FIFO queues.
...
+
.The screenshot shows the message row, the message payload in the details pane, and the *Return Messages* icon.
...
.The arrow shows the trash can icon in the details pane for deleting a message in the message browser.
image::mq-msg-browser-delete-icon.png[Trash can icon for deleting a message in the Message Browser]
...
In the Purge Messages page, select the checkbox, and then *Delete Messages* to verify that you want to purge all messages in the queue:
...
+
. In the Delete Queue page, select the checkbox to verify that you want to delete the queue, and then click *Delete Queue*:
+
...
. In the *Message Sender*, specify the message group to which the message belongs:
+
...
+
image::mq-fifo-msg-group-id.png[]
