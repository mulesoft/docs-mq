...
See <<dead-letter-queues>>.
** *Enable Cross-Region Failover* creates a fallback queue in the fallback region.
See <<fallback-queues>>.
--
. Click *Create Queue*.
+ 
...
Before you can enable fallback on a standard queue that has a DLQ assigned,
the DLQ must have had fallback enabled.
// (more info here)
...
For information about cross-region failover and fallback queues, see
