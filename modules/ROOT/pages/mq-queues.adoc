...
IMPORTANT: If multiple messages are sent to a FIFO queue containing the same message ID, the first message to arrive is kept as the valid message. Subsequent messages are considered duplicates and are discarded as they arrive due to the deduplication feature built into FIFO.
