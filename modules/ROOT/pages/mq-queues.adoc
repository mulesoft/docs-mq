...
+
See xref:mq-understanding.adoc#encrypt_queue[Encrypted queues].
...
* Shouldn't be used with `prefetch` mode for applications with long processing times that use the `maxConcurrency` attribute to limit concurrent processing.
+
These options can cause messages to remain in the buffer longer, which causes the messages to expire and be sent back to the queue.
