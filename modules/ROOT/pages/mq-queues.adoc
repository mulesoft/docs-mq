...
+
.The screenshot shows (*1*) the message row, (*2*) the message payload in the details pane, (*3*) *Return Messages* icon, (*4*) *Delete* icon, (*5*) *Full Screen* icon, and (*6*) close details pane icon (*X*).
image::mq-msg-browser-payload-details.png[Selected message row, payload, Return Messages icon, Delete icon, Full Screen icon, and close details pane icon (X)]
+
...
The subscriber can't specify to receive receive messages with a specific message group ID.
