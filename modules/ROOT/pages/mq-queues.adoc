...
+
. In the *Delete Queue* screen, click the checkbox:
+
...
* Should not be use with `prefetch` mode.
...
Message groups enable multiple consumers to access messages in a FIFO queue. A single consumer can access messages in a message group at a time. Message order is preserved within the message group.

You use the message group ID to specify the message group that the message belongs to. 

The FIFO queue processes messages in the message group one by one, in order. Messages with different message group IDs might be processed out of order.

If multiple applications send messages associated with the same message group ID to a FIFO queue, the messages are stored in the order they arrive. To preserve the order, make sure that each app uses a unique message group ID to send messages.

If one consumer accesses multiple message groups, the entire FIFO queue is blocked.
