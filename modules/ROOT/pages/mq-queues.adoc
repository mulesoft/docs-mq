...
** *Encryption* (optional) encrypts all messages in the queue. See xref:mq-understanding.adoc#encrypt_queue[Encrypted queues].
+
...
* You can set delivery delay on all messages in a FIFO queue. Delivery delay is not supported for individual messages in a FIFO queue. If you change the delivery delay for a FIFO queue, the setting change is retroactive to all messages in the queue.
