...
For more information about FIFO queues, see:
...
. Click the queue ID or queue name in the details pane to open the Messaging feature:
+
image::mq-queue-details.png[]
+
...
+
If you specified *Assign Default Delivery Delay* for the queue, you see the default delivery delay.
. Click *Send*.

== Verify the Message in a Queue

To verify that the message arrived in the queue, either return to the *Destinations* screen to observe the number of messages in the queue, or you can assume the message is in the queue, and you can <<Get a Message From a Queue, get a message from the queue>>.
...
. In the *Destinations* screen, click the queue type for the queue to display its details in the right pane.
. Click the queue to see the queue detail in the right pane. The detail shows that a message is in
the queue:
+
