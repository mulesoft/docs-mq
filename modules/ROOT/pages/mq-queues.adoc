...
+
If you change the delivery delay for a FIFO queue, the setting change is retroactive to all messages in the queue.
...
* Can be used as a DLQ; however, it is not recommended. See the limitations.
+
If you assign a DLQ to a FIFO queue, the DLQ must also be a FIFO queue.
...
* Can't guarantee that messages in a message group are processed in order when used with a DLQ.
+
...
+
Therefore, to ensure that messages in the message group are processed in order,
don't use a FIFO queue with a DLQ.
