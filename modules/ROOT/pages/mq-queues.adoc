...
** *Encryption* encrypts all messages in the queue. See xref:mq-understanding.adoc#encrypt-queue[Encrypted queues].
...
+
.The *Destinations* page shows the ID, type, and details label for the queue.
...
+
.The screenshot shows (*1*) the message row, (*2*) the message payload in the details pane, and (*3*) the *Return Messages* icon.
image::mq-msg-browser-payload-details.png[Selected message row, payload in the details pane, and *Return Messages* icon]

If you want to return the message to the queue (for example, to enable other applications to read the message), click the *Return Messages* icon. 
...
.The arrow shows the trash can icon in the details pane for deleting a message in the queue.
image::mq-msg-browser-delete-icon.png[Trash can icon for deleting a message in the queue]
...
+
.The screenshot shows (*1*) the message row, (*2*) the message payload in the details pane, and (*3*) the *Return Messages* icon.
image::mq-msg-browser-payload-details-json.png[Selected message row, JSON payload in the details pane, and *Return Messages* icon]
...
+
.The screenshot shows (*1*) the message row, (*2*) the message payload in the details pane, and (*3*) the *Return Messages* icon.
image::mq-msg-browser-payload-details-csv.png[Selected message row, CSV payload in the details pane, and *Return Messages* icon]
...
. In the *Purge Messages* confirmation popup, select the checkbox, and then click *Delete Messages* to verify that you want to purge all messages in the queue:
+
...
. In *Anypoint Platform > MQ*, click *Destinations* in the left pane.
. Click the queue type for the queue to display its details in the right pane.
. Click the trash can icon in the upper right of the details pane:
+
.The arrow shows the trash can icon in the details pane for deleting a queue.
image::mq-delete-queue-icon.png[Trash can icon in the details pane for deleting a queue]
+
. In the *Delete Queue* confirmation popup, select the checkbox, and then click *Delete Queue* to verify that you want to delete the queue:
+
