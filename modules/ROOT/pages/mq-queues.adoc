...
See also <<fifoqueues,About FIFO Queues>>.
...
+
. Click Queue, Exchange, or FIFO Queue. For Exchange, see xref:mq-exchanges.adoc[To Configure Message Exchanges].
...
*Note:* If you need to delete a queue, see xref:mq-faq.adoc#how-do-i-delete-a-queue[How do I delete a queue?]
...
* See the xref:index.adoc[Anypoint MQ starting page] for the regions MQ and FIFO are available in.
...
* xref:release-notes::mq/mq-release-notes.adoc#version-1-5-0-january-28-2017[Release Notes] reference.
...
You can also view this from the REST Administration API from the Get Queue REST endpoint. If DLQ is set, the returned entities contain the deadLetterSources field. For more information, see xref:mq-apis.adoc#mqadminapi[MQ Administration REST API].
...
== Verifying the Message in a Queue
...
+
Alternatively, you can delete the message by clicking the trash can icon. In Anypoint MQ, deleting a message is called an `ack` - For information on how Anypoint MQ processes ack  messages for you, see xref:mq-ack-mode.adoc[Acknowledgement Mode].
+
