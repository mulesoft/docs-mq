...
In the Configure Queue section, there are 2 bullets: one explaining what standard queues are, and another one for FIFO queues. I believe this is the right place to explain that standard queues delivery reliability level is "at-least-once". In the FIFO queue description, it is explicitly stated that the delivery reliability level is "exactly-once", which is fine
